...
public class WristSetAngleWithVisionAuto extends Command {
   ...
   */
  public WristSetAngleWithVisionAuto(Wrist wrist, AllianceSelection allianceSelection, DriveTrain drivetrain, FileLog log) {
    this.wrist = wrist;
  ...
  private double getAngleFromDistance(int n) {
    ...
    double correctionOffset = (n == 3) ? (3.79204*dist - 12.7572) : 0;
  ...
  public boolean isFinished() {
    return !wrist.isEncoderCalibrated() ||  Math.abs(wrist.getWristAngle() - wrist.getCurrentWristTarget()) < 3.0;
  }
}
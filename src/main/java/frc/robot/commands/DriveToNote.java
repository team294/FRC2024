...
public class DriveToNote extends Command {
  ...
  private final JeVoisCamera camera;
  ...
  /**
   * @param camera JeVois camera subsystem
   * @param driveTrain drive train subsystem to use
   * @param log filelog to use
   */
  public DriveToNote(JeVoisCamera camera, DriveTrain driveTrain, FileLog log) {
    this.driveTrain = driveTrain;
    this.log = log;
    this.camera = camera;
    fwdRateController = new PIDController(0.01, 0, 0);
    leftRateController = new PIDController(0.001, 0, 0);
    turnRateController = new PIDController(0.01, 0, 0);
    fwdRateController.setSetpoint(JeVoisConstants.height); // setpoint is the bottom of the screen
    leftRateController.setSetpoint(JeVoisConstants.width / 2); // setpoint is the middle line on the screen
    turnRateController.setSetpoint(JeVoisConstants.width / 2); // setpoint is the middle line on the screen
  ...
  public void execute() {
    ...
    fwdVelocity = fwdRateController.calculate(camera.getClosestTargetY());
    leftVelocity = leftRateController.calculate(camera.getClosestTargetX() - (JeVoisConstants.width / 2));
    turnRate = turnRateController.calculate(camera.getClosestTargetX());

...
public class CenterFourNearNoteShoot extends SequentialCommandGroup {
  /** Creates a new CenterFourNearNoteShoot. */
  public CenterFourNearNoteShoot(Intake intake, Shooter shooter, DriveTrain driveTrain, Feeder feeder, BCRRobotState robotState, TrajectoryCache cache, AllianceSelection alliance, FileLog log) {
    ...
    addCommands(
      ...
      new ParallelCommandGroup(
        new IntakePieceAuto(intake, feeder, robotState, log),
        new ConditionalCommand(
          new DriveTrajectory(CoordType.kAbsolute, StopType.kBrake, cache.cache[TrajectoryType.driveFromCenterNoteToSourceNoteRed.value], driveTrain, log), 
          new DriveTrajectory(CoordType.kAbsolute, StopType.kBrake, cache.cache[TrajectoryType.driveFromCenterNoteToSourceNoteBlue.value], driveTrain, log), 
          () -> alliance.getAlliance() == Alliance.Red
        )
      ),
      new ShootPiece(shooter, feeder, robotState, log)

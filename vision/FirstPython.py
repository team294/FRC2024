...
class FirstPython:
    ...
    def __init__(self):
        ...
        self.HSVmin = np.array([ 2,  160, 150], dtype=np.uint8)
        self.HSVmax = np.array([ 13, 255, 255], dtype=np.uint8)
        

        # Measure your U-shaped object (in meters) and set its size here:
        self.owm = 0.356 # width in meters
        self.ohm = 0.356 # height in meters

        # Other processing parameters:
        self.epsilon = 0.015               # Shape smoothing factor (higher for smoother)
        self.hullarea = ( 50*50, 10000*10000 ) # Range of object area (in pixels) to track
        self.hullfill = 3                 # Max fill ratio of the convex hull (percent)
    ...
    def detect(self, imgbgr, outimg = None):
        ...
        # imgblurhsv = cv2.bilateralFilter(imghsv,10,50,50)
        ...
        # Only consider the 3 biggest objects by area:
